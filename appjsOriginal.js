...
app.get('/RequestStartingPage', (req, res) => {
    res.sendFile(path.join(__dirname, 'public/RequestStartingPage.html'));
});
...
app.post('/NewInvestor', upload.fields([{ name: 'Image', maxCount: 1 }]), (req, res) => {
    ...
    db.query(sql, [InvestorID, InvestorName, PhoneNumber, ImageFile, BankDetails], (err) => {
        ...
        // ✅ Update NoOfInvestors in Company table
        db.query(`UPDATE Company 
                  SET NoOfInvestors = NoOfInvestors + 1
                  WHERE 1`, (updateErr) => {
            if (updateErr) {
                console.error('Error updating company investor count:', updateErr);
                return res.status(500).send('Investor added but failed to update company data');
            }

            console.log('Investor added and company updated');
            res.send(`
                <script>
...
app.post("/SubmitLoanRequest",(req,res)=>{
    ...
    const {ClientName,LoanAmount,ClientPhoneNumber,Years,Months,Days,Collateral} = req.body;
    const Duration=parseInt(Years)*365+parseInt(Months)*30+parseInt(Days);
    ...
    // Execute the query, passing values from the request body
    db.query(sql, [ ClientName,LoanAmount,Duration,ClientPhoneNumber,Collateral], (err) => {
        if (err) {
            console.error('Error inserting record:', err);
            res.status(500).send('Error inserting record');
            return;
...
app.post("/SubmitInvestmentRequest",(req,res)=>{
    const {InvestorName,InvestmentAmount,InvestorPhoneNumber} = req.body;
    ...
    // Execute the query, passing values from the request body
    db.query(sql, [ InvestorName,InvestmentAmount,InvestorPhoneNumber], (err) => {
        if (err) {
            console.error('Error inserting record:', err);
            res.status(500).send('Error inserting record');
            return;
...
app.get('/getMonthlyInvestmentOrders', async (req, res) => {
    try {
        ...
        // Combined Investments (Open + Closed)
        const [investmentsData] = await db.promise().query(`
            SELECT 
                MONTH(dateField) AS month,
                SUM(amountField) AS totalInvestments
            FROM (
                SELECT InvestmentDate AS dateField, AmountSanctioned AS amountField FROM Investments WHERE YEAR(InvestmentDate) = ?
                UNION ALL
                SELECT StartDate AS dateField, Amount AS amountField FROM closedInvestments WHERE YEAR(StartDate) = ?
            ) AS AllInvestments
            GROUP BY MONTH(dateField)
            ORDER BY MONTH(dateField);
        `, [currentYear, currentYear]);

        // Combined Orders (Open + Closed)
        const [ordersData] = await db.promise().query(`
            SELECT 
                MONTH(dateField) AS month,
                SUM(amountField) AS totalOrders
            FROM (
                SELECT StartDate AS dateField, AmountSanctioned AS amountField FROM Orders WHERE YEAR(StartDate) = ?
                UNION ALL
                SELECT StartDate AS dateField, Amount AS amountField FROM closedOrders WHERE YEAR(StartDate) = ?
            ) AS AllOrders
            GROUP BY MONTH(dateField)
            ORDER BY MONTH(dateField);
        `, [currentYear, currentYear]);
        ...
        const investments = Array(12).fill(0);
        const orders = Array(12).fill(0);

        investmentsData.forEach(row => investments[row.month - 1] = row.totalInvestments || 0);
        ordersData.forEach(row => orders[row.month - 1] = row.totalOrders || 0);
    ...
    try {
        const [investmentsData] = await db.promise().query(`
            SELECT 
                ...
                SUM(amountField) AS totalInvestments
            FROM (
                SELECT InvestmentDate AS dateField, AmountSanctioned AS amountField FROM Investments
                UNION ALL
                SELECT StartDate AS dateField, Amount AS amountField FROM closedInvestments
            ) AS AllInvestments
        ...
        const [ordersData] = await db.promise().query(`
            SELECT 
                ...
                SUM(amountField) AS totalOrders
            FROM (
                SELECT StartDate AS dateField, AmountSanctioned AS amountField FROM Orders
                UNION ALL
                SELECT StartDate AS dateField, Amount AS amountField FROM closedOrders
            ) AS AllOrders
            GROUP BY YEAR(dateField)
            ORDER BY YEAR(dateField);
        `);
        ...
        const investments = Array(allYears.length).fill(0);
        const orders = Array(allYears.length).fill(0);

        investmentsData.forEach(row => {
            const index = allYears.indexOf(row.year);
...
app.get('/getQuarterlyInvestmentOrders', async (req, res) => {
    try {
        ...
        const labels = ["Q1", "Q2", "Q3", "Q4"].map(q => `${year}-${q}`);
        const investments = [0, 0, 0, 0];
